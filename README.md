...
| `proxy` | string | — | Route every request through a forward proxy — an `http(s)://` CONNECT proxy or a `socks5://` proxy, optionally with `user:pass@` auth (≤2048 chars). The trusted residential-IP egress for IP-blocked sites (issue #22); the corpus passes it only on a bot-block retry. Transport only — the SSRF guard still resolves and vets the target host before the proxy hop. |
