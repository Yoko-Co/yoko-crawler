...
Only a crawl that reached **no page of the site** is reclassified: any crawl that fetched a real page is left `completed` with `failure_reason: null`. Note that is "no page", not "no response" — since #102 a crawl whose only responses were seeding traffic (robots.txt, sitemaps, sitemap probes) is failed as `seeding_incomplete`, because those rows are not an inventory of anything. Rollback for that specific reclassification is `YOKO_CRAWL_FAIL_SEEDING_INCOMPLETE=0`, which keeps the diagnostic log and drops the failure.
...
- Seeds from `async def start()` (Scrapy ≥ 2.13's entry point) — **not** `start_requests()`, which Scrapy 2.17 removed along with every call site. `requirements.txt` pins `scrapy>=2.18,<3`: while it was unbounded, an ordinary upgrade silently made the old seeding dead code, so robots.txt (and therefore sitemap discovery) was never fetched. The floor is **2.18** rather than 2.13 for a second instance of the same failure class — the `depth_reset` meta key (#81), which an older Scrapy ignores in silence, leaving the start URL at depth 1 and every page below it off by one
...
| `YOKO_CRAWL_ROBOTS_TIMEOUT` | No | Per-attempt budget for fetching robots.txt, in seconds. Default 60, **bounded to 60–600** (issue #92). Below the floor is refused, not clamped quietly: a shorter budget does not speed a crawl up, it makes a slow site look like one with no robots.txt and the crawl then proceeds allow-all against a site that may have said `Disallow`. Above 600 is clamped, because robots.txt is the crawl's only seed and the pre-page stall is 3 × the budget (× redirect hops × sitemap probes) — past the ceiling that outruns the job manager's 7500s watchdog. **Check `restrictions.knobs` on `GET /crawl/{id}` to see whether your value was honoured**: `disposition` is one of `default` (unset), `honoured` (set and accepted as-is), `raised`, `floored`, `clamped`, `invalid` (not a usable number) or `unknown` (the spider never got far enough to report). The disposition is the field that matters, not the number: a `floored` value produces the identical effective budget to an unset one, so the number alone cannot tell you whether your setting took effect. |
| `YOKO_CRAWL_MAX_ROBOTS_DELAY` | No | Cap on the robots.txt `Crawl-delay` we will honour, in seconds (default 10). A site asking for more is paced at the cap, and the clamp is logged and surfaced in `restrictions.crawl_delay`. Unlike the knob above it falls back silently rather than logging — `restrictions.knobs.max_crawl_delay.disposition` reads `invalid` when that happens, which covers a non-number, a non-positive value, and `inf`/`nan` (all of which `float()` accepts, and a cap of infinity is not a cap). |
