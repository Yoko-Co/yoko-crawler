...
class TestStartCrawl:
    ...
    async def test_start_crawl_forwards_user_agent(self, client, auth_headers):
        mock_process = AsyncMock()
        mock_process.returncode = None
        mock_process.pid = 12345
        ...
        assert response.status_code == 202
        args = mock_exec.call_args.args
        # UA override rides on argv.
        assert args[args.index("--user-agent") + 1] == "Mozilla/5.0 (X11; Linux) Chrome/131"

    async def test_start_crawl_rejects_control_chars_in_user_agent(self, client, auth_headers):
        # CRLF in the UA could inject a header downstream -> 422, not accepted.
        with patch("domain_validator.asyncio.get_running_loop") as mock_loop:
            ...
            response = await client.post(
                "/crawl",
                json={"domain": "example.com", "user_agent": "Mozilla/5.0\r\nEvil: 1"},
                headers=auth_headers,
            )
        assert response.status_code == 422
